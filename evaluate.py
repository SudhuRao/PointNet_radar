...
parser.add_argument('--model_path', default='D:/Sudhu/Project/codes/my_pointnet/Data_npoint_z0/points_1024/log/model.ckpt', help='model checkpoint file path [default: log/model.ckpt]')
parser.add_argument('--dump_dir', default='dump', help='dump folder path [dump]')
parser.add_argument('--visu', action='store_true',default=True, help='Whether to dump image for error case [default: False]')
...
NUM_CLASSES = 4
SHAPE_NAMES = ["Car","TwoWheeler","Pedestrian","Coke"] 

HOSTNAME = socket.gethostname()

## ModelNet40 official train/test split
#TRAIN_DIR = 'D:/Sudhu/Project/codes/my_pointnet/data/Train'
...
#    data_train[file,...]  = pd.read_csv(path).values 
#    
...
TEST_DIR = 'D:/Sudhu/Project/codes/my_pointnet/Data_npoint_z0/points_1024/Test'
...
for file in range(0,len(TEST_FILES)):
    ...
    if(bool(re.match("(.*car.*)",path))):
        label_test[file] = 0
    elif(bool(re.match("(.*TwoWheeler.*)|(.*twowheeler.*)",path))):
        label_test[file] = 1
    elif(bool(re.match(".*Pedestrian.*",path))):
        label_test[file] = 2
    elif(bool(re.match(".*Coke.*",path))):
    ...
    for batch_idx in range(num_batches):
        ...
        for vote_idx in range(num_votes):
            for file in range(start_idx,end_idx):
                ...
                rotated_data[idx,...] = provider.rotate_point_cloud_by_angle(temp_data,
                                              vote_idx/float(num_votes) * np.pi * 2)
                idx = idx + 1

            feed_dict = {ops['pointclouds_pl']: rotated_data[0:BATCH_SIZE, 0:NUM_POINT, :],
        ...
        for i in range(start_idx, end_idx):
            ...
            if pred_val[i-start_idx] == l and FLAGS.visu: # ERROR CASE, DUMP!
                ...
                output_img = pc_util.point_cloud_three_views(np.squeeze(data_test[i]))
